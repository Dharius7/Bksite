...
import { isEmail, isPositiveAmount, trimRequired } from '@/lib/validation';
...
export default function WiseTransferPage() {
  ...
  const transferSummary = useMemo(() => {
    const parts = [
      `Wise: ${fullName || 'N/A'} (${email || 'N/A'})`,
      `Country: ${country || 'N/A'}`,
      note ? `Note: ${note}` : null,
    ].filter(Boolean);
    return parts.join(' | ');
  }, [country, email, fullName, note]);
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      const payload = {
        toAccount: email.trim(),
        amount: Number(amount),
        description: transferSummary,
        method: 'wise',
      };
      const response = await api.post('/transfers', payload);
      setMessage(response.data?.message || 'Wise transfer submitted successfully');
      await fetchBalance();
      setAmount('');
      setFullName('');
      setEmail('');
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!trimRequired(fullName) || !isEmail(email)) {
      setError('Please enter a valid name and email.');
      return;
    }
    if (!trimRequired(country)) {
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-6 py-6 text-white">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex-1 text-center">
              ...
              <h2 className="text-lg font-semibold">Wise Transfer</h2>
              <p className="text-sm text-white/80">Low-cost international money transfers with Wise.</p>
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-5 md:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="flex justify-center">
            <div className="bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-gray-900">Wise Transfer</div>
                <div className="text-xs text-gray-500">Formerly TransferWise - Low fee international transfers</div>
              </div>
            </div>
          </div>
