...
>>>>>>> b2ccfa7 (First Update commit)
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      const payload = {
        toAccount: cashTag.trim(),
        amount: Number(amount),
        description: transferSummary,
        method: 'cashapp',
      };
      const response = await api.post('/transfers', payload);
      setMessage(response.data?.message || 'Cash App transfer submitted successfully');
      await fetchBalance();
      setAmount('');
      setCashTag('');
      setFullName('');
...
=======
    ...
    if (!trimRequired(cashTag) || !trimRequired(fullName)) {
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-6 py-6 text-white">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex-1 text-center">
              ...
              <h2 className="text-lg font-semibold">Cash App Transfer</h2>
              <p className="text-sm text-white/80">Send money using Cash App.</p>
            </div>
