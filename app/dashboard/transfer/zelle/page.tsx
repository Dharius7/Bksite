...
import { isEmail, isPhone, isPositiveAmount, trimRequired } from '@/lib/validation';
...
export default function ZelleTransferPage() {
  ...
  const transferSummary = useMemo(() => {
    const parts = [
      `Zelle Email: ${zelleEmail || 'N/A'}`,
      `Phone: ${phone || 'N/A'}`,
      `Name: ${fullName || 'N/A'}`,
      note ? `Note: ${note}` : null,
    ].filter(Boolean);
    return parts.join(' | ');
  }, [fullName, note, phone, zelleEmail]);
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      const payload = {
        toAccount: zelleEmail.trim() || phone.trim(),
        amount: Number(amount),
        description: transferSummary,
        method: 'zelle',
      };
      const response = await api.post('/transfers', payload);
      setMessage(response.data?.message || 'Zelle transfer submitted successfully');
      await fetchBalance();
      setAmount('');
      setZelleEmail('');
      setPhone('');
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!trimRequired(fullName) || !isEmail(zelleEmail) || !isPhone(phone)) {
        ...
        <div className="bg-gradient-to-r from-blue-700 to-blue-500 px-6 py-6 text-white">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex-1 text-center">
              ...
              <h2 className="text-lg font-semibold">Zelle Transfer</h2>
              <p className="text-sm text-white/80">Send money using Zelle.</p>
            </div>
